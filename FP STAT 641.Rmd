...
# Research Questions
* What is the relationship between the response (Weekly sales) and all other predictors? Compare the standard error and 95% confidence interval of the classical ordinary least square (OLS) fitted model with the residual resampling and observational resampling methods.

* Conduct hypothesis testing using the response (Weekly sales) and the factor (Holiday Flag) with an infer package. Compare the results with student's two-sample t test for mean difference.
...
# Student's two-sample t test for Mean difference
```{r}
# Create a variable called Holiday.Week that filters data using 
# the Walmart dataset pipeline.
Holiday.Week <- Walmart %>%
                        dplyr::select(Weekly_Sales,Holiday_Flag) %>%
                        filter(Holiday_Flag == 1)

# Create a variable called Non.Holiday.Week that filters data using 
# the Walmart dataset pipeline.
Non.holiday.Week <- Walmart %>%
                       dplyr::select(Weekly_Sales,Holiday_Flag) %>%
                        filter(Holiday_Flag == 0) 

# On vectors of data, use the t.test() function to perform a student's two-sample t-test.
t.test(x = Holiday.Week$Weekly_Sales, y = Non.holiday.Week$Weekly_Sales, alternative = "greater", var.equal = TRUE)
```

I used the two-sample t-test under the non-normality assumption to determine the difference in the achieved significance p-value. Following Fisher's theory, Fisher supported the use of the student's two-sample t-test under non-normality assumption. I discovered that under the non-normality and equal variance assumptions, the permutation achieved significance level (ASL) / p-value using an infer package is identical to the t-test (ASL). 
